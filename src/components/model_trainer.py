...
class ModelTrainer:
    def __init__(self, train_data_path=None, test_data_path=None, trainer_config=None):
        self.trainer_config = ModelTrainerConfig()
        if train_data_path!=None:
            self.trainer_config.train_data_path = train_data_path
        if test_data_path!=None:
            self.trainer_config.test_data_path = test_data_path
    ...
    def evaluate(self, model, parameters, model_name):
        try :
            ...
            f1 = f1_score(train_data['type'].values, predicted)
            ...
            if f1 > f1_prev:
                ...
                logging.info(f"{model_name} underperforms the previous optimal model! Keeping the previous model...")

            logging.info(f"Model Evaluation for {model_name} has completed")
            # print("F1_Score :", f1)
